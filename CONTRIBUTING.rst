...
Report bugs at https://github.com/HDI-Project/DataAudit/issues.
...
DataAudit could always use more documentation, whether as part of the
official DataAudit docs, in docstrings, or even on the web in blog posts,
articles, and such.

Submit Feedback
~~~~~~~~~~~~~~~

The best way to send feedback is to file an issue at https://github.com/HDI-Project/DataAudit/issues.
...
Ready to contribute? Here's how to set up `DataAudit` for local development.

1. Fork the `DataAudit` repo on GitHub.
2. Clone your fork locally::

    $ git clone git@github.com:your_name_here/DataAudit.git

3. Install your local copy into a virtualenv. Assuming you have virtualenvwrapper installed,
   this is how you set up your fork for local development::

    $ mkvirtualenv DataAudit
    $ cd DataAudit/
    $ pip install -e .
...
5. The pull request should work for Python2.7, 3.4, 3.5 and 3.6. Check
   https://travis-ci.org/HDI-Project/DataAudit/pull_requests
...
2. The tests that cover a module called ``dataaudit/path/to/a_module.py`` should be
   implemented in a separated module called ``tests/dataaudit/path/to/test_a_module.py``.
...
2. Update the version in ``setup.cfg``, ``dataaudit/__init__.py`` and ``HISTORY.md`` files.
3. Create a new TAG pointing at the correspoding commit in ``stable`` branch.
4. Merge the new commit from ``stable`` into ``master``.
5. Update the version in ``setup.cfg`` and ``dataaudit/__init__.py`` to open the next
