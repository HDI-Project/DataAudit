...
lint: ## check style with flake8 and isort
	flake8 dataaudit tests
	isort -c --recursive dataaudit tests

fixlint: ## fix lint issues using autoflake, autopep8, and isort
	find dataaudit -name '*.py' | xargs autoflake --in-place --remove-all-unused-imports --remove-unused-variables
	autopep8 --in-place --recursive --aggressive dataaudit
	isort --apply --atomic --recursive dataaudit
...
coverage: clean-coverage ## check code coverage quickly with the default Python
	coverage run --source dataaudit -m pytest
	coverage report -m
	coverage html
	$(BROWSER) htmlcov/index.html

clean-docs: ## remove previously built docs
	rm -f docs/dataaudit.rst
	rm -f docs/dataaudit.*.rst
	rm -f docs/modules.rst
	$(MAKE) -C docs clean

docs: clean-docs ## generate Sphinx HTML documentation, including API docs
	sphinx-apidoc -o docs/ dataaudit
	$(MAKE) -C docs html
